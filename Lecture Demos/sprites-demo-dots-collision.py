...
while playing:
	...
	for event in pygame.event.get():
		if event.type == pygame.MOUSEBUTTONDOWN:
			...
			playing = False
	dots.update(500, 500)
	collide_dict = pygame.sprite.groupcollide(dots, dots, False, False)
	for dot in collide_dict:
		for collide_dot in collide_dict[dot]:
			if dot is not collide_dot:
				dot.ySpeed *=-1
				dot.xSpeed *= -1
